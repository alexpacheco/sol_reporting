...
daily_summary <- daily %>%  filter(Day >= as.Date("2019-10-01") & Day < as.Date("2020-10-01")) %>%
  group_by(Day) %>%
...
dailypwrconsump <- dailypower %>%  filter(Date >= as.Date("2019-10-01") & Date < as.Date("2020-10-01"))
...
monthlyreport <- monthly %>% ay1920filter %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport <- monthly %>% ay1920filter %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport <- monthly %>% ay1920filter %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport <- monthly %>% ay1920filter %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage <- monthly %>% ay1920filter %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

users <- monthly %>% ay1920filter %>%
  group_by(Name) %>%
  summarize(Total=sum(as.double(Total)))

pis <- monthly %>% ay1920filter %>%
  group_by(PI) %>%
  summarize(Total=sum(as.double(Total)),Jobs=sum(as.double(TotalJ)))

pidept <- monthly %>% ay1920filter %>%
  group_by(PIDept) %>%
  summarize(Total=sum(as.double(Total)))

dept <- monthly %>% ay1920filter %>%
  group_by(Department) %>%
  summarize(Total=sum(as.double(Total)))
...
```{r ay1920_users_sus}
monthly %>% ay1920filter %>% topuserstotal 
```
...
```{r ay1920_users_jobs}
monthly %>% ay1920filter %>% topusersjobs
```

### Top PIs

```{r ay1920_pis_sus-jobs}
monthly %>% ay1920filter %>%
...
usage <- round(sum(monthlyreport$Total)/sum(ay1920su[1:months_sofar])*100,2)
monthlyreport %>% 
