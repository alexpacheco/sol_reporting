CY 2018 {data-navmenu="Annual Reports"}
=====================================================================

Row
-----------------------------------------------------------------------

```{r cy2018_monthly_setup}
daily_summary <- daily %>%  filter(Day >= as.Date("2018-01-01") & Day < as.Date("2019-01-01")) %>%
  group_by(Day) %>%
...
dailypwrconsump <- dailypower %>%  filter(Date >= as.Date("2018-01-01") & Date < as.Date("2019-01-01"))

data_tmp <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2018-01-01") & as.Date(V1) < min(as.Date("2019-01-01"),today()))
...
dateWindow = c(as.Date("2018-01-01"), min(as.Date("2018-12-31"),today()) )

monthlyreport <- monthly %>% cy2018filter %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport <- monthly %>% cy2018filter %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport <- monthly %>% cy2018filter %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport <- monthly %>% cy2018filter %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage <- monthly %>% cy2018filter %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

users <- monthly %>% cy2018filter %>%
  group_by(Name) %>%
  summarize(Total=sum(as.double(Total)))

pis <- monthly %>% cy2018filter %>%
  group_by(PI) %>%
  summarize(Total=sum(as.double(Total)),Jobs=sum(as.double(TotalJ)))

pidept <- monthly %>% cy2018filter %>%
  group_by(PIDept) %>%
  summarize(Total=sum(as.double(Total)))

dept <- monthly %>% cy2018filter %>%
  group_by(Department) %>%
  summarize(Total=sum(as.double(Total)))
...
fortusage <- round(forttotal/(sum(cy2018su[1:months_sofar]))*100)
...
```{r cy2018_users_sus}
monthly %>% cy2018filter %>% topuserstotal 
```
...
```{r cy2018_users_jobs}
monthly %>% cy2018filter %>% topusersjobs
```

### Top PIs

```{r cy2018_pis_sus-jobs}
monthly %>% cy2018filter %>%
...
```{r cy2018_monthly_usage}
monthlyreport %>% 
  mutate(Available=cy2018su[1:months_sofar],Unused=Available-Total,Percent=round(Total/Available*100,2)) %>%
...
```{r cy2018_monthly_usage_percent}
usage <- round(sum(monthlyreport$Total)/sum(cy2018su[1:months_sofar])*100,2)
monthlyreport %>% 
  mutate(Available=cy2018su[1:months_sofar],
  ...
  plot_ly(x = ~Month, y = ~Percent, type = "bar", name = "Consumed" ) %>%
    ...
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack',
      ...
      x0=as.Date("2018-01-01"), x1=min(Sys.Date(),as.Date("2018-12-01")), 
...
```{r cy2018_monthly_power}
averagepower <- monthlypower %>% cy2018filter %>%
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower %>% cy2018filter %>%
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
