...
daily_summary <- daily %>%  filter(Day >= as.Date("2020-10-01") & Day < as.Date("2021-10-01")) %>%
  group_by(Day) %>%
...
dailypwrconsump <- dailypower %>%  filter(Date >= as.Date("2020-10-01") & Date < as.Date("2021-10-01"))
...
monthlyreport <- monthly %>% ay2021filter %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport <- monthly %>% ay2021filter %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport <- monthly %>% ay2021filter %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport <- monthly %>% ay2021filter %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage <- monthly %>% ay2021filter %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

users <- monthly %>% ay2021filter %>%
  group_by(Name) %>%
  summarize(Total=sum(as.double(Total)))

pis <- monthly %>% ay2021filter %>%
  group_by(PI) %>%
  summarize(Total=sum(as.double(Total)),Jobs=sum(as.double(TotalJ)))

pidept <- monthly %>% ay2021filter %>%
  group_by(PIDept) %>%
  summarize(Total=sum(as.double(Total)))

dept <- monthly %>% ay2021filter %>%
  group_by(Department) %>%
  summarize(Total=sum(as.double(Total)))
...
```{r ay2021_users_sus}
monthly %>% ay2021filter %>% topuserstotal 
```
...
```{r ay2021_users_jobs}
monthly %>% ay2021filter %>% topusersjobs
```

### Top PIs

```{r ay2021_pis_sus-jobs}
monthly %>% ay2021filter %>%
...
usage <- round(sum(monthlyreport$Total)/sum(ay2021su[1:months_sofar])*100,2)
monthlyreport %>% 
