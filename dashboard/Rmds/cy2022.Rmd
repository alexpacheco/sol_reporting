...
daily_summary <- daily %>%  filter(Day >= as.Date("2022-01-01") & Day < as.Date("2023-01-01")) %>%
  group_by(Day) %>%
...
dailypwrconsump <- dailypower %>%  filter(Date >= as.Date("2022-01-01") & Date < as.Date("2023-01-01"))
...
monthlyreport <- monthly %>% cy2022filter %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport <- monthly %>% cy2022filter %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport <- monthly %>% cy2022filter %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport <- monthly %>% cy2022filter %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage <- monthly %>% cy2022filter %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

users <- monthly %>% cy2022filter %>%
  group_by(Name) %>%
  summarize(Total=sum(as.double(Total)))

pis <- monthly %>% cy2022filter %>%
  group_by(PI) %>%
  summarize(Total=sum(as.double(Total)),Jobs=sum(as.double(TotalJ)))

pidept <- monthly %>% cy2022filter %>%
  group_by(PIDept) %>%
  summarize(Total=sum(as.double(Total)))

dept <- monthly %>% cy2022filter %>%
  group_by(Department) %>%
  summarize(Total=sum(as.double(Total)))
...
```{r cy2022_users_sus}
monthly %>% cy2022filter %>% topuserstotal 
```
...
```{r cy2022_users_jobs}
monthly %>% cy2022filter %>% topusersjobs
```

### Top PIs

```{r cy2022_pis_sus-jobs}
monthly %>% cy2022filter %>%
...
usage <- round(sum(monthlyreport$Total)/sum(cy2022su[1:months_sofar])*100,2)
monthlyreport %>% 
  ...
  plot_ly(x = ~Month, y = ~Percent, type = "bar", name = "Consumed" ) %>%
    ...
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack',
      ...
      x0=as.Date("2021-12-31"), x1=min(Sys.Date(),as.Date("2022-12-01")), 
...
averagepower <- monthlypower %>% cy2022filter %>%
  group_by(Month) %>%
...
monthlypower %>% cy2022filter %>%
  ...
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2022-01-01"), x1=min(Sys.Date(),as.Date("2022-12-01")),
