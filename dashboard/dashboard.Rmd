...
data[is.na(data)] <- 0
datagpu <- read.table('/home/alp514/usage/dailygpu.csv', header = FALSE, sep = ",", col.names = paste0("V",seq_len(11)), fill = TRUE)
...
monthlypower2122 <- read_csv('monthlypower2122.csv')
dailypower2122 <- read_csv('dailypower2122.csv')
monthlypower2021 <- read_csv('monthlypower2021.csv')
dailypower2021 <- read_csv('dailypower2021.csv')
...
dailypower1617 <- read_csv('dailypower1617.csv') %>% mutate(Power=Energy/24) %>% select(c(Date,Power,Energy))
monthlypower <- rbind(monthlypower1617,monthlypower1718,monthlypower1819,monthlypower1920,monthlypower2021,monthlypower2122) %>%
  group_by(Month) %>% summarize(Power=sum(Power), Energy=sum(Energy))
dailypower <- rbind(dailypower1718,dailypower1819,dailypower1920,dailypower2021,dailypower2122) %>%
...
all <- lts + im1080 + eng + engc + himem + enge + engi + im2080 + chem + health + hawk + infolab

solnodes <- ltsnodes + im1080nodes + engnodes + engcnodes + himemnodes + engenodes + enginodes + engecnodes + im2080nodes + chemnodes + healthnodes + infolabnodes
hawknodes <- hawkcpunodes + hawkmemnodes + hawkgpunodes
totalnodes <- solnodes + hawknodes
solcores <- ltsnodes*ltscores + (im1080nodes + engnodes)*engcores + engcnodes*engccores + himemnodes*himemcores + (engenodes + enginodes + engecnodes + im2080nodes + chemnodes + healthnodes )*engecores + infolabnodes*hawkcpucores
hawkcores <- (hawkcpunodes + hawkmemnodes)*hawkcpucores + hawkgpunodes*hawkgpucores
totalcores <- solcores + hawkcores
solmem <- (ltsnodes + im1080nodes + engnodes) * 128 + engcnodes * 64 + himemnodes * 512 + (engenodes + enginodes + engecnodes + im2080nodes + chemnodes + healthnodes)  * 192 + infolabnodes * 384
...
solperf <- (ltsnodes*ltscores*ltsperf + (im1080nodes + engnodes)*im1080cores*im1080perf + engcnodes*engccores*engcperf + himemnodes*himemcores*himemperf + 
     ...
     infolabnodes*hawkcpucores*hawkcpuperf/1000 + hawkgpunodes*hawkgpucores*hawkgpuperf/1000
...
solgpumem <- im1080gpu * 8 + im2080gpu * 11
...
solgpuperf <- im1080gpu*257/1000 + im2080gpu*367.2/1000
...
* Sol is a `r solnodes` node heterogeneous cluster launched on Oct 1, 2016 initially with `r ltsnodes+im1080nodes` nodes.
   ...
   * `r im1080gpu+im2080gpu` GPUs (`r sprintf("%6d",im1080cuda+im2080cuda)` CUDA Cores) with `r solgpumem`GB memory providing a total performance of `r solgpuperf` TFLOPs.
...
* Total Performance
   ...
   * `r im1080gpu+im2080gpu+hawkgpu` GPUs (`r sprintf("%6d",im1080cuda+im2080cuda+teslat4cuda)` CUDA Cores) with `r gpumem`GB memory providing a total performance of `r totalgpuperf` TFLOPs.
...
knitr::include_graphics('./sol_value.jpg')
```
...
```{r ay1617_usage_all}
data_1617 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2016-10-01") & as.Date(V1) < as.Date("2017-10-01"))
datagpu_1617 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2016-10-01") & as.Date(V1) < as.Date("2017-10-01"))
data1617 <- full_join(data_1617,datagpu_1617)
dayslice <- length(data1617$V1)
d1 <- seq(as.POSIXct(data1617[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2016-10-01"), as.Date("2017-09-30") )

dygraph( xts(cbind(data1617$V2,data1617$V12), order.by = d1)) %>%
	dySeries("V1", label = "Total", color = "blue") %>%
	dySeries("V2", label = "GPU", color = "green") %>%
	dyAxis("y", label = "SUs consumed") %>%
...
averagepower1617 <- monthlypower1617 %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower1617 %>% 
  group_by(Month) %>%
  ...
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2017-06-01"), x1=as.Date("2017-09-01"),
      y0= mean(averagepower1617$Energy), y1= mean(averagepower1617$Energy), line=list(width=2, color = "green")))
    )
```
...
```{r ay1718_usage_all}
data_1718 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2017-10-01") & as.Date(V1) < as.Date("2018-10-01"))
datagpu_1718 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2017-10-01") & as.Date(V1) < as.Date("2018-10-01"))
data1718 <- full_join(data_1718,datagpu_1718)
dayslice <- length(data1718$V1)
d1 <- seq(as.POSIXct(data1718[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2017-10-01"), as.Date("2018-09-30") )

dygraph( xts(cbind(data1718$V2,data1718$V12), order.by = d1)) %>%
	dySeries("V1", label = "Total", color = "blue") %>%
	dySeries("V2", label = "GPU", color = "green") %>%
	dyAxis("y", label = "SUs consumed") %>%
	dyRoller(rollPeriod = 7) %>%
...
averagepower1718 <- monthlypower1718 %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower1718 %>% 
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2017-10-01"), x1=as.Date("2018-09-01"),
      y0= mean(averagepower1718$Energy), y1= mean(averagepower1718$Energy), line=list(width=2, color = "green")))
    )
```
...
averagepower1819 <- monthlypower1819 %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower1819 %>% 
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2018-10-01"), x1=as.Date("2019-09-01"),
      y0= mean(averagepower1819$Energy), y1= mean(averagepower1819$Energy), line=list(width=2, color = "green")))
    )
```
...
```{r ay1920_usage_all}
data_1718 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2019-10-01") & as.Date(V1) < as.Date("2020-10-01"))
datagpu_1718 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2019-10-01") & as.Date(V1) < as.Date("2020-10-01"))
data1718 <- full_join(data_1718,datagpu_1718)
dayslice <- length(data1718$V1)
d1 <- seq(as.POSIXct(data1718[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2019-10-01"), as.Date("2020-09-30") )
...
averagepower1920 <- monthlypower1920 %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower1920 %>% 
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
     shapes=list(list(type='line',
      x0=as.Date("2019-10-01"), x1=as.Date("2020-09-01"),
      y0= mean(averagepower1920$Energy), y1= mean(averagepower1920$Energy), line=list(width=2, color = "green")))
    )
```
...
AY 2020-21 {data-navmenu="Annual Reports"}
=====================================================================

Row
-----------------------------------------------------------------------

```{r ay2021_monthly_setup}
monthlyreport2021 <- monthly2021 %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport2021 <- monthly2021 %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport2021 <- monthly2021 %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport2021 <- monthly2021 %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage2021 <- monthly2021 %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))
...
months_sofar <- length(monthlyreport2021$Month)

forttotal <- sum(pis$Total)
fortusage <- round(forttotal/(sum(ay2021su[1:months_sofar]))*100)
fortjobs <- sum(pis$Jobs)
```
...
```{r ay2021_usage_all}
data_1718 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2020-10-01") & as.Date(V1) < as.Date("2021-10-01"))
datagpu_1718 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2020-10-01") & as.Date(V1) < as.Date("2021-10-01"))
data1718 <- full_join(data_1718,datagpu_1718)
dayslice <- length(data1718$V1)
d1 <- seq(as.POSIXct(data1718[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2020-10-01"), as.Date("2021-09-30") )
...
```{r ay2021_monthly_usage}
monthlyreport2021 %>% 
  mutate(Available=ay2021su[1:months_sofar],Unused=Available-Total,Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Total, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack')
```
...
```{r ay2021_monthly_usage_percent}
usage2021 <- round(sum(monthlyreport2021$Total)/sum(ay2021su[1:months_sofar])*100,2)
monthlyreport2021 %>% 
  mutate(Available=ay2021su[1:months_sofar],
     Unused=round((Available-Total)/Available*100,2),
     Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Percent, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack',
      shapes=list(list(type='line', 
      x0=as.Date("2020-10-01"), x1=as.Date("2021-09-01"), 
      y0= usage2021, y1= usage2021, line=list(width=2, color = "green")),
      list(type='line', 
      x0=as.Date("2020-10-01"), x1=as.Date("2021-09-01"), 
      y0= 80, y1= 80, line=list(width=2, color = "red")))
    )
...
```{r ay2021_monthly_power}
averagepower2021 <- monthlypower2021 %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower2021 %>% 
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2020-10-01"), x1=as.Date("2021-09-01"),
      y0= mean(averagepower2021$Energy), y1= mean(averagepower2021$Energy), line=list(width=2, color = "green")))
    )
```
...
```{r ay2021_pis_sus}
plot_ly(pireport2021, x = ~Total, y = ~reorder(PI, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
     yaxis = list(title = '' ),
     margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2021_pis_users}
plot_ly(pireport2021, values = ~User, labels = ~PI, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F)
```
...
```{r ay2021_pidept_sus}
plot_ly(pideptreport2021, x = ~Total, y = ~reorder(PIDept, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
     yaxis = list(title = '' ),
     margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2021_pidept_users}
plot_ly(pideptreport2021, values = ~User, labels = ~PIDept, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F)
```
...
```{r ay2021_userdept_sus}
plot_ly(deptreport2021, x = ~Total, y = ~reorder(Department, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
   yaxis = list(title = '' ),
   margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2021_userdept_users}
plot_ly(deptreport2021, values = ~User, labels = ~Department, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```
...
```{r ay2021_users_status_sus}
plot_ly(userusage2021, x = ~Total, y = ~Status, type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = ' ', type = 'log' ), 
   yaxis = list(title = '' ),
   margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2021_users_status_num}
plot_ly(userusage2021, values = ~User, labels = ~Status, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```


CY 2021 {data-navmenu="Annual Reports"}
=====================================================================

Row
-----------------------------------------------------------------------

```{r cy2021_monthly_setup}
monthlyreport2021 <- monthly_2021 %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport2021 <- monthly_2021 %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport2021 <- monthly_2021 %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport2021 <- monthly_2021 %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage2021 <- monthly_2021 %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))
...
forttotal <- sum(pis$Total)
fortusage <- round(forttotal/(sum(cy2021su[1:months_sofar]))*100)
fortjobs <- sum(pis$Jobs)
```
...
```{r cy2021_usage_all}
data_1718 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2021-01-01") & as.Date(V1) < as.Date("2022-01-01"))
datagpu_1718 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2021-01-01") & as.Date(V1) < as.Date("2022-01-01"))
data1718 <- full_join(data_1718,datagpu_1718)
dayslice <- length(data1718$V1)
d1 <- seq(as.POSIXct(data1718[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2021-01-01"), as.Date("2021-12-31") )
...
```{r cy2021_monthly_usage}
monthlyreport2021 %>% 
  mutate(Available=cy2021su[1:months_sofar],Unused=Available-Total,Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Total, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack')
```
...
```{r cy2021_monthly_usage_percent}
usage2021 <- round(sum(monthlyreport2021$Total)/sum(cy2021su[1:months_sofar])*100,2)
monthlyreport2021 %>% 
  mutate(Available=cy2021su[1:months_sofar],
     Unused=round((Available-Total)/Available*100,2),
     Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Percent, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack'
      , shapes=list(list(type='line', 
      x0=as.Date("2021-01-01"), x1=min(Sys.Date(),as.Date("2021-10-31")), 
      y0= usage2021, y1= usage2021, line=list(width=2, color = "green"))
      ,list(type='line', 
      x0=as.Date("2021-01-01"), x1=min(Sys.Date(),as.Date("2021-12-31")),
      y0= 80, y1= 80, line=list(width=2, color = "red"))
    )
    )
```
...
```{r cy2021_monthly_power}
averagepower2021 <- monthlypower %>% cy2021filter %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower %>% cy2021filter %>%
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
...
```{r cy2021_users_status_num}
plot_ly(userusage2021, values = ~User, labels = ~Status, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```


AY 2021-22 {data-navmenu="Annual Reports"}
=====================================================================

Row
-----------------------------------------------------------------------

```{r ay2122_monthly_setup}
monthlyreport2122 <- monthly2122 %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport2122 <- monthly2122 %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport2122 <- monthly2122 %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport2122 <- monthly2122 %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage2122 <- monthly2122 %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))
...
months_sofar <- length(monthlyreport2122$Month)

forttotal <- sum(pis$Total)
fortusage <- round(forttotal/(sum(ay2122su[1:months_sofar]))*100)
fortjobs <- sum(pis$Jobs)
```
...
```{r ay2122_usage_all}
data_1718 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2021-10-01") & as.Date(V1) < min(as.Date("2022-10-01"),today()))
datagpu_1718 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2021-10-01") & as.Date(V1) < min(as.Date("2022-10-01"),today()))
data1718 <- full_join(data_1718,datagpu_1718)
dayslice <- length(data1718$V1)
d1 <- seq(as.POSIXct(data1718[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2021-10-01"), min(as.Date("2022-09-30"),today()) )
...
```{r ay2122_monthly_usage}
monthlyreport2122 %>% 
  mutate(Available=ay2122su[1:months_sofar],Unused=Available-Total,Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Total, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack')
```
...
```{r ay2122_monthly_usage_percent}
usage2122 <- round(sum(monthlyreport2122$Total)/sum(ay2122su[1:months_sofar])*100,2)
monthlyreport2122 %>% 
  mutate(Available=ay2122su[1:months_sofar],
     Unused=round((Available-Total)/Available*100,2),
     Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Percent, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack'
      , shapes=list(list(type='line', 
      x0=as.Date("2021-10-01"), x1=min(Sys.Date(),as.Date("2022-09-01")), 
      y0= usage2122, y1= usage2122, line=list(width=2, color = "green"))
      ,list(type='line', 
      x0=as.Date("2021-10-01"), x1=min(Sys.Date(),as.Date("2022-09-01")),
      y0= 80, y1= 80, line=list(width=2, color = "red"))
    )
...
```{r ay2122_monthly_power}
averagepower2122 <- monthlypower2122 %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower2122 %>% 
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2021-10-01"), x1=min(Sys.Date(),as.Date("2022-09-01")),
      y0= mean(averagepower2122$Energy), y1= mean(averagepower2122$Energy), line=list(width=2, color = "green")))
    )
```
...
```{r ay2122_pis_sus}
plot_ly(pireport2122, x = ~Total, y = ~reorder(PI, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
     yaxis = list(title = '' ),
     margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2122_pis_users}
plot_ly(pireport2122, values = ~User, labels = ~PI, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F)
```
...
```{r ay2122_pidept_sus}
plot_ly(pideptreport2122, x = ~Total, y = ~reorder(PIDept, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
     yaxis = list(title = '' ),
     margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2122_pidept_users}
plot_ly(pideptreport2122, values = ~User, labels = ~PIDept, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F)
```
...
```{r ay2122_userdept_sus}
plot_ly(deptreport2122, x = ~Total, y = ~reorder(Department, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
   yaxis = list(title = '' ),
   margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2122_userdept_users}
plot_ly(deptreport2122, values = ~User, labels = ~Department, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```
...
```{r ay2122_users_status_sus}
plot_ly(userusage2122, x = ~Total, y = ~Status, type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = ' ', type = 'log' ), 
   yaxis = list(title = '' ),
   margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r ay2122_users_status_num}
plot_ly(userusage2122, values = ~User, labels = ~Status, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```


CY 2022 {data-navmenu="Annual Reports"}
=====================================================================

Row
-----------------------------------------------------------------------

```{r cy2022_monthly_setup}
monthlyreport2022 <- monthly_2022 %>% 
  group_by(Month) %>%	
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

pireport2022 <- monthly_2022 %>% 
  group_by(PI) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ)))) 

pideptreport2022 <- monthly_2022 %>% 
  group_by(PIDept) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

deptreport2022 <- monthly_2022 %>% 
  group_by(Department) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))

userusage2022 <- monthly_2022 %>% 
  group_by(Status) %>% 
  summarize(Total=round(sum(as.double(Total)),2),User=n_distinct(Name),Jobs=round(sum(as.double(TotalJ))))
...
months_sofar <- length(monthlyreport2022$Month)

forttotal <- sum(pis$Total)
fortusage <- round(forttotal/(sum(cy2022su[1:months_sofar]))*100)
fortjobs <- sum(pis$Jobs)
```
...
```{r cy2022_usage_all}
data_1718 <- data %>% select(V1,V2) %>%
	filter(as.Date(V1) >= as.Date("2022-01-01") & as.Date(V1) < min(as.Date("2023-01-01"),today()))
datagpu_1718 <-  datagpu %>% mutate(V12=V2+V4+V6+V8+V10) %>% select(V1,V12) %>%
	filter(as.Date(V1) >= as.Date("2022-01-01") & as.Date(V1) < min(as.Date("2023-01-01"),today()))
data1718 <- full_join(data_1718,datagpu_1718)
dayslice <- length(data1718$V1)
d1 <- seq(as.POSIXct(data1718[1,1]), by = "days", length = dayslice)
dateWindow = c(as.Date("2022-01-01"), min(as.Date("2022-12-31"),today()) )
...
```{r cy2022_monthly_usage}
monthlyreport2022 %>% 
  mutate(Available=cy2022su[1:months_sofar],Unused=Available-Total,Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Total, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack')
```
...
```{r cy2022_monthly_usage_percent}
usage2022 <- round(sum(monthlyreport2022$Total)/sum(cy2022su[1:months_sofar])*100,2)
monthlyreport2022 %>% 
  mutate(Available=cy2022su[1:months_sofar],
     Unused=round((Available-Total)/Available*100,2),
     Percent=round(Total/Available*100,2)) %>%
  plot_ly(x = ~Month, y = ~Percent, type = "bar", name = "Consumed" ) %>%
    add_trace(y =  ~ Unused, name = "Unused") %>%
    layout(yaxis = list(title = 'SUs Consumed' ), xaxis = list(title = 'Month' ), barmode = 'stack'
      , shapes=list(list(type='line', 
      x0=as.Date("2022-01-01"), x1=min(Sys.Date(),as.Date("2022-12-31")), 
      y0= usage2022, y1= usage2022, line=list(width=2, color = "green"))
      ,list(type='line', 
      x0=as.Date("2022-01-01"), x1=min(Sys.Date(),as.Date("2022-12-31")),
      y0= 80, y1= 80, line=list(width=2, color = "red"))
    )
...
```{r cy2022_monthly_power}
averagepower2022 <- monthlypower %>% cy2022filter %>% group_by(Month) %>% summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2))
monthlypower %>% cy2022filter %>%
  group_by(Month) %>%
  summarize(Power=round(sum(Power)/1000,2),Energy=round(sum(Energy)/1000,2)) %>%
  plot_ly(x = ~Month, y = ~Energy, type = "bar", name = "Energy Consumption" ) %>%
    layout(yaxis = list(title = 'Energy Consumed (MWh)' ), xaxis = list(title = 'Month' ),
      shapes=list(list(type='line',
      x0=as.Date("2022-01-01"), x1=min(Sys.Date(),as.Date("2022-12-31")),
      y0= mean(averagepower2022$Energy), y1= mean(averagepower2022$Energy), line=list(width=2, color = "green")))
    )
```
...
```{r cy2022_pis_sus}
plot_ly(pireport2022, x = ~Total, y = ~reorder(PI, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
     yaxis = list(title = '' ),
     margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r cy2022_pis_users}
plot_ly(pireport2022, values = ~User, labels = ~PI, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F)
```
...
```{r cy2022_pidept_sus}
plot_ly(pideptreport2022, x = ~Total, y = ~reorder(PIDept, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
     yaxis = list(title = '' ),
     margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r cy2022_pidept_users}
plot_ly(pideptreport2022, values = ~User, labels = ~PIDept, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F)
```
...
```{r cy2022_userdept_sus}
plot_ly(deptreport2022, x = ~Total, y = ~reorder(Department, Total), type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = '', type = 'log' ), 
   yaxis = list(title = '' ),
   margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r cy2022_userdept_users}
plot_ly(deptreport2022, values = ~User, labels = ~Department, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```
...
```{r cy2022_users_status_sus}
plot_ly(userusage2022, x = ~Total, y = ~Status, type = "bar", name = "PI Usage Report", orientation = 'h') %>%
  layout(xaxis = list(title = ' ', type = 'log' ), 
   yaxis = list(title = '' ),
   margin = list(l = 200, r = 20, t = 25, b = 25))
```
...
```{r cy2022_users_status_num}
plot_ly(userusage2022, values = ~User, labels = ~Status, type = "pie", textposition = 'inside', textinfo = 'label', showlegend = F) 
```
...
AY 2020-21 {data-navmenu="Job Types"}
===========================================================
...
```{r ay2021_pi_sus_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  ay2021filter %>%
  summarize(Serial=sum(as.double(Serial)),
...
```{r ay2021_pi_jobs_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  ay2021filter %>%
  summarize(SerialJ=sum(as.double(SerialJ)),
...
AY 2021-22 {data-navmenu="Job Types"}
===========================================================
...
```{r ay2122_pi_sus_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  ay2122filter %>%
  summarize(Serial=sum(as.double(Serial)),
...
```{r ay2122_pi_jobs_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  ay2122filter %>%
  summarize(SerialJ=sum(as.double(SerialJ)),
...
CY 2021 {data-navmenu="Job Types"}
===========================================================
...
```{r cy2021_pi_sus_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  cy2021filter %>%
  summarize(Serial=sum(as.double(Serial)),
...
```{r cy2021_pi_jobs_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  cy2021filter %>%
  summarize(SerialJ=sum(as.double(SerialJ)),
...
CY 2022 {data-navmenu="Job Types"}
===========================================================
...
```{r cy2022_pi_sus_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  cy2022filter %>%
  summarize(Serial=sum(as.double(Serial)),
...
```{r cy2022_pi_jobs_percent_jobtype}
monthly %>%
  group_by(PI) %>%
  cy2022filter %>%
  summarize(SerialJ=sum(as.double(SerialJ)),
