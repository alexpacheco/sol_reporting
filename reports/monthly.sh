...
totalalloc=$(echo $ltsalloc $im1080alloc $engalloc $engcalloc $himemalloc $engealloc $engialloc $im2080alloc | awk '{for (i=1;i<=NF;i++){s+=$i}}END{print s}')
total=0
...
echo "========================================"
echo "Usage by PI"
for pi in $(sacct -a --state=COMPLETED,CANCELLED,FAILED,TIMEOUT --starttime=$start --endtime=$end -X -o Account%15  | tail -n +3 | sort | awk -F_ '{print $1}' | uniq )
do
  usage=$(sacct -a -A ${pi}_${ay},${pi}_${ayold},${pi}_s${ay} --state=COMPLETED,CANCELLED,FAILED,TIMEOUT --starttime=$start --endtime=$end -X -o Partition,CPUTimeRAW | \
