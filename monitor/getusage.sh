...
squeue --format="$squeuefmt" --states=PENDING,RUNNING | egrep -v 'vdi|singularity' | sed -e 's/gpu://g' -e 's/TRES_PER_NODE/GPUs/g' | sed 's/\r//' > queue.csv
#squeue --format="$squeueFMT" --states=PENDING,RUNNING | egrep -v 'vdi|singularity' > queue.csv
sacct -a --state="COMPLETED,CANCELLED,FAILED,TIMEOUT,PREEMPTED,NODE_FAIL" -S $(date -d "today" +%Y-%m-%d-00:00:00) -E $(date -d "now" +%Y-%m-%d-%H:%M:%S) -p --delimiter=";" \
...
sinfo -N --format="$sinfofmt" -p lts,im1080,eng,engc,enge,himem-long,engi,im2080,chem,health,infolab,hawkcpu,hawkmem,hawkgpu | sed -e 's_N/A_0.0_g' -e 's/himem-long/himem/g' > load.csv

sinfo -N --format="%.10N;%.12R;%.5G" -p lts-gpu,eng-gpu,im1080-gpu,im2080-gpu,hawkgpu | sed -e 's/gpu://g' | egrep -v null > gpuinfo.csv
squeue --format="%.20N;%.12P;%.5b" -p lts-gpu,eng-gpu,im1080-gpu,im2080-gpu,hawkgpu -t RUNNING | sed -e 's/gpu://g' | egrep -v 'null|N/A'  > gpuload.csv
