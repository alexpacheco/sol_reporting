...
jobsthisay2122 <- read_delim('jobsalloc.csv.gz',delim=";") %>% filter(End >= as.Date("2022-01-01") & End < as.Date("2023-01-01"))
jobsthismonth <- read_delim('jobs-0.csv',delim=";")#, col_select=c(JobID:End)) 
day_of_month <- mday(today())
if ( day_of_month > 1 ){
  jobsthisay <- rbind(jobsthisay2122,jobsthismonth) %>% separate(Account, c("Account", "year1", "year2") , sep="_", fill = "right") %>% select(-c("year1","year2"))
}
...
ay2022su <- lts+im1080+eng+engc+himem+enge+engi+im2080+chem+health+hawkcpu+hawkgpu+hawkmem+infolab
if ( Sys.Date() > as.Date("2022-12-31")){
  currmonth <- length(seq(from=as.Date("2022-01-01"), to=as.Date("2022-12-31"), by='month'))
} else {
  if ( day_of_month > 1 ){
    currmonth <- length(seq(from=as.Date("2022-01-01"), to=Sys.Date(), by='month'))
  } else {
    currmonth <- length(seq(from=as.Date("2022-01-01"), to=Sys.Date()-1, by='month'))
  }
}
...
read_csv('../dashboard/monthlypower.csv') %>% 
  ...
  select(c(Year,Month,Partition,Host,Power,Energy)) -> monthlypower2022
read_csv('../dashboard/dailypower.csv') %>% 
  ...
  select(c(Date,Year,Month,Day,Partition,Host,Power,Energy)) -> dailypower2022
...
```{r annualsummary}
jobsthisay %>% summary %>% 
   mutate(Partition="Total",Used=round(SU/sum(ay2022su[1:currmonth])*100,2)) %>%
...
jobsthisay %>% monthly_summary %>% partpercent(ay2022su[1:currmonth]) -> total
...
```{r monthlyenergy-lts}
monthlypower2022 %>% filter(str_detect(Partition,'lts')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage(kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'im1080')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
```{r monthyenergy-eng}
monthlypower2022 %>% filter(Partition=="eng" | Partition=="eng-gpu") %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'engc')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
#monthlypower2022 %>% filter(str_detect(Partition,'himem')) %>%
#  group_by(Year,Month) %>%
#  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
#  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'enge')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'engi')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'im2080')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'chem')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'health')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'hawkcpu')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'hawkgpu') | str_detect(Partition,'hawktest')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'hawkmem')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
monthlypower2022 %>% filter(str_detect(Partition,'infolab')) %>%
  group_by(Year,Month) %>%
  summarize('Average Power Usage (kW)'=round(sum(Power)/1000,2),'Energy Consumption (MWh)'=round(sum(Energy)/1000,2)) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```
...
```{r october,echo=F}
monthly_usage( jobsthisay, as.Date("2022-10-01"), as.Date("2022-11-01") )
```

### November

```{r november,echo=F}
monthly_usage( jobsthisay, as.Date("2022-11-01"), as.Date("2022-12-01") )
```

### December

```{r december,echo=F}
monthly_usage( jobsthisay, as.Date("2022-12-01"), as.Date("2023-01-01") )
```
