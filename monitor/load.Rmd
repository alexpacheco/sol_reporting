...
output:
  flexdashboard::flex_dashboard:
    ...
    navbar:
      ...
      - { title: "AY 21-22 Report", href: "https://webapps.lehigh.edu/hpc/monitor/ay2122.html"} 
...
colnames(queue_status) =  c("Job ID","Partition","Job Name","User","Allocation","State",
   ...
   "Nodes Requested","Memory/Core Requested","Nodelist","Reason","Priority","GPUs")
...
jobshistory <- read_delim('jobshistory.csv.gz',delim=";") %>% filter(Time >= today() - months(12))
...
health <- Sol %>% filter(Partition=="health")
hawkcpu <- Sol %>% filter(Partition=="hawkcpu")
hawkgpu <- Sol %>% filter(Partition=="hawkgpu")
hawkmem <- Sol %>% filter(Partition=="hawkmem")
infolab <- Sol %>% filter(Partition=="infolab")
pisces <- Sol %>% filter(Partition=="pisces")
ima40-gpu <- Sol %>% filter(Partition=="ima40-gpu")
```
...
```{r}
gauge(round(health$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#health-1")
```

### hawkcpu

```{r}
gauge(round(hawkcpu$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#hawkcpu-1")
```

### hawkgpu

```{r}
gauge(round(hawkgpu$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#hawkgpu-1")
```

### hawkmem

```{r}
gauge(round(hawkmem$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#hawkmem-1")
```

### infolab

```{r}
gauge(round(infolab$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#infolab-1")
```

### pisces

```{r}
gauge(round(pisces$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#pisces-1")
```

### ima40

```{r}
gauge(round(ima40$Used), min = 0, max = 100, symbol = '%', gaugeSectors(colors = c('red')), href="#ima40-1")
```
...
```{r}
Sol %>% datatable(rowname=FALSE, options = list(dom = 't', pageLength = 25, autoWidth = TRUE),
...
```{r}
queue_status %>% 
  filter(Partition=="lts" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
...
```{r}
queue_status %>% 
  filter(Partition=="lts-gpu" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="lts" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="im1080" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="im1080-gpu" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="im1080" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="eng" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="eng-gpu" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="eng" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="engc" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="engc" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(str_detect(Partition, "himem") & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(str_detect(Partition,"himem") & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="enge" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="enge" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="engi" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="engi" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="im2080" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="im2080-gpu" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="im2080" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(str_detect(Partition,"chem") & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
#  filter(trimws(Partition)=="chem" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(str_detect(Partition,"chem") & State=="PENDING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
#  filter(trimws(Partition)=="chem" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```

health {data-navmenu="Partitions"}
===================================
...
### health cpu usage per node

```{r}
clean_nodeload %>% filter(trimws(Partition)=="health") %>%
  plot_ly(y = ~Nodes, x = ~Allocated, name = "Used", type = "bar", marker = list( color = colors[1]) ) %>%
  add_trace(x = ~Idle, name = "Idle", marker = list( color = colors[2]) ) %>%
  add_trace(x = ~Drained, name = "Drained", marker = list( color = colors[3])) %>%
  layout(yaxis = list(title = '' ), xaxis = list(title = 'Cores', dtick = 6 ), barmode = 'stack')
...
```{r}
queue_status %>% 
  filter(Partition=="health" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="health" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```


hawkcpu {data-navmenu="Partitions"}
===================================
...
### hawkcpu cpu usage per node

```{r}
clean_nodeload %>% filter(trimws(Partition)=="hawkcpu") %>%
  plot_ly(y = ~Nodes, x = ~Allocated, name = "Used", type = "bar", marker = list( color = colors[1]) ) %>%
  add_trace(x = ~Idle, name = "Idle", marker = list( color = colors[2]) ) %>%
  add_trace(x = ~Drained, name = "Drained", marker = list( color = colors[3])) %>%
  layout(yaxis = list(title = '' ), xaxis = list(title = 'Cores', dtick = 6 ), barmode = 'stack')
...
```{r}
queue_status %>% 
  filter(Partition=="hawkcpu" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="hawkcpu" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```


hawkgpu {data-navmenu="Partitions"}
===================================
...
### hawkgpu cpu usage per node

```{r}
clean_nodeload %>% filter(trimws(Partition)=="hawkgpu") %>%
  plot_ly(y = ~Nodes, x = ~Allocated, name = "Used", type = "bar", marker = list( color = colors[1]) ) %>%
  add_trace(x = ~Idle, name = "Idle", marker = list( color = colors[2]) ) %>%
  add_trace(x = ~Drained, name = "Drained", marker = list( color = colors[3])) %>%
  layout(yaxis = list(title = '' ), xaxis = list(title = 'Cores', dtick = 6 ), barmode = 'stack')
...
### hawkgpu gpu usage per node

```{r}
gpuload %>% filter(str_detect(Partition,"hawkgpu")) %>%
  mutate(Idle=(Available-Usage)) %>%
  plot_ly(y = ~Node, x = ~Usage, name = "Used", type = "bar", marker = list( color = colors[1]) ) %>%
  add_trace(x = ~Idle, name = "Available", marker = list( color = colors[2]) ) %>%
  layout(yaxis = list(title = '' ), xaxis = list(title = 'GPUs', dtick = 1 ), barmode = 'stack')
...
```{r}
queue_status %>% 
  filter(Partition=="hawkgpu" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="hawkgpu" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```


hawkmem {data-navmenu="Partitions"}
===================================
...
### hawkmem cpu usage per node

```{r}
clean_nodeload %>% filter(trimws(Partition)=="hawkmem") %>%
  plot_ly(y = ~Nodes, x = ~Allocated, name = "Used", type = "bar", marker = list( color = colors[1]) ) %>%
  add_trace(x = ~Idle, name = "Idle", marker = list( color = colors[2]) ) %>%
  add_trace(x = ~Drained, name = "Drained", marker = list( color = colors[3])) %>%
  layout(yaxis = list(title = '' ), xaxis = list(title = 'Cores', dtick = 6 ), barmode = 'stack')
...
```{r}
queue_status %>% 
  filter(Partition=="hawkmem" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="hawkmem" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```



infolab {data-navmenu="Partitions"}
===================================
...
### infolab cpu usage per node

```{r}
clean_nodeload %>% filter(trimws(Partition)=="infolab") %>%
  plot_ly(y = ~Nodes, x = ~Allocated, name = "Used", type = "bar", marker = list( color = colors[1]) ) %>%
  add_trace(x = ~Idle, name = "Idle", marker = list( color = colors[2]) ) %>%
  add_trace(x = ~Drained, name = "Drained", marker = list( color = colors[3])) %>%
  layout(yaxis = list(title = '' ), xaxis = list(title = 'Cores', dtick = 6 ), barmode = 'stack')
...
```{r}
queue_status %>% 
  filter(Partition=="infolab" & State=="RUNNING") %>% select(-c(Partition,State,Reason,Priority,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
```{r}
queue_status %>% 
  filter(Partition=="infolab" & State=="PENDING") %>% select(-c(Partition,State,`Elapsed Time`,Nodelist,GPUs)) %>%
  datatable(filter = 'top', options = list(pageLength = 100))
```
...
clean_nodeload %>% 
  ...
  datatable(options = list(pageLength = 200, dom = 't'))
```
