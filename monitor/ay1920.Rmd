...
if ( month(today()) == 10 ){
  jobsthisay <- read_delim('jobs-0.csv',delim=";")
} else { 
  jobsthisay <- read_delim('jobsalloc.csv.gz',delim=";") 
...
ay1920su <- lts+im1080+eng+engc+himem+enge+engi+im2080+chem
if ( day_of_month > 1 ){
  currmonth <- length(seq(from=as.Date("2019-10-01"), to=Sys.Date(), by='month'))
} else {
  currmonth <- length(seq(from=as.Date("2019-10-01"), to=Sys.Date()-1, by='month'))
}
...
rbind(Total_SU_Jobs,ltstotal,im1080total,engtotal,engctotal,himemtotal,engetotal,engitotal,im2080total,chemtotal) %>%
...
rbind(serialtotal,lts_serial,im1080_serial,eng_serial,engc_serial,himem_serial,enge_serial,engi_serial,im2080_serial,chem_serial) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive") )
```
...
rbind(smptotal,lts_smp,im1080_smp,eng_smp,engc_smp,himem_smp,enge_smp,engi_smp,im2080_smp,chem_smp) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive") )
```
...
rbind(dmptotal,lts_dmp,im1080_dmp,eng_dmp,engc_dmp,himem_dmp,enge_dmp,engi_dmp,im2080_dmp,chem_dmp) %>%
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive") )
```
...
jobsthisay %>% monthly_summary %>% partpercent(ay1920su[1:currmonth]) -> total
